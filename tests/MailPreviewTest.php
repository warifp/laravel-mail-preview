...
class MailPreviewTest extends TestCase
{
    ...
    public function testCreatesPreviewFiles()
    {
        ...
        $files->shouldReceive('put')->with(
            'framework/emails/'.$message->getDate()->getTimestamp().'_'.$message->getDate()->format('u').'_me_at_example_com_foo_subject.html',
            m::any()
        );

        $files->shouldReceive('put')->with(
            'framework/emails/'.$message->getDate()->getTimestamp().'_'.$message->getDate()->format('u').'_me_at_example_com_foo_subject.eml',
            $message->toString()
        );
